...
import { StockHolding, DividendPayment, TaxRefund } from './types';
...
function saveStoreItem<T>(storeName: string, item: T): Promise<void> {
  if (useMemoryFallback) {
    const list = (memoryStore as any)[storeName];
    if (list) {
      const idx = list.findIndex((x: any) => x.id === (item as any).id || (storeName === 'settings' && x.key === (item as any).key));
      if (idx !== -1) {
        list[idx] = item;
      } else {
        list.push(item);
...
export const portafolioDB = {
  ...
  async clearAllData(): Promise<void> {
    ...
    await clearStore('dividends');
    await clearStore('refunds');
    // We don't necessarily clear settings, but we can set default
